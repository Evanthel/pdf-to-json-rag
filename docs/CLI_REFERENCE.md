...
`corpus-sanity-check` samples the repo-local `pdf/` corpus through `pdf/lcwa_gov_pdf_metadata.csv`, runs isolated workflow checks on the sampled PDFs, and returns aggregate rates, bucket-level diagnostics, follow-up actions with concrete failure examples, a saved snapshot path, a corpus contract gate, and a corpus architecture gate over `processing`, `semantics`, and `trust`. Use `--sample-profile quick|balanced|stress` to control cost; `--sample-size` still overrides the profile when needed.
...
`runtime-check --json` reports install context, the requested embedding backend, effective backend/model, local sentence-transformer availability, fallback reason, cross-encoder opt-in state, and LLM synthesis opt-in state.
